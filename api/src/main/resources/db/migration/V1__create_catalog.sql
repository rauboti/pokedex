...
-- module (gamedata). Upsert by stable source `id`; rows are never deleted (a form
-- vanishing from the source keeps existing collection rows valid). `synced_at` records
-- the last sync that wrote each row. `move` is created before `species` because
-- species carries the recommended-moveset FKs.

-- Move: one fast or charged move (data-model.md). Ranking inputs (power/energy/
-- duration_ms) drive the sync-time moveset ranker; `type` feeds STAB and matchup
-- display.
create table move (
    id          text             primary key,
    name        text             not null,
...
-- registration search filters on it. The recommended-move FKs are computed at sync
-- and NULL when the pool is empty/unknown. Base-stat changes across a resync trigger
-- the staleness rescan.
...
-- species are replaced on sync (delete+insert per species inside the sync transaction).
-- `legacy` marks moves not currently obtainable by normal means (legacy/Elite TM). A
-- recorded move that later leaves the pool stays on the Pokémon: caught_pokemon FKs
-- point at `move`, not `species_move`.
