...
class StalenessRescanTest : IntegrationTest() {
    ...
    fun `a rebalance flags the affected caught Pokemon stale and leaves the unaffected one alone`() {
        ...
        val venusaur = registerPerfectPokemon("VENUSAUR", venusaurCp, 40.0)
        val charmander = registerPerfectPokemon("CHARMANDER", charmanderCp, 20.0)

        every { gamedataClient.fetchPokedex() } returns rebalanced
        syncService.sync()

        assertThat(caughtPokemonService.findByUserIdAndId(venusaur, ownerId)!!.stale).isTrue()
        assertThat(caughtPokemonService.findByUserIdAndId(charmander, ownerId)!!.stale).isFalse()
    }
    ...
    fun `the caller-scoped catalog stale count reflects the rescan`() {
        every { gamedataClient.fetchPokedex() } returns fixture
        syncService.sync()
        registerPerfectPokemon("VENUSAUR", venusaurCp, 40.0)
        registerPerfectPokemon("CHARMANDER", charmanderCp, 20.0)
    ...
    private fun registerPerfectPokemon(
        speciesId: String,
        cp: Int,
        level: Double,
    ): UUID =
        caughtPokemonService
            .save(
                CaughtBasePokemon(
                    userId = ownerId,
                    speciesId = speciesId,
