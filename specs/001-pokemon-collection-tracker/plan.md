...
| IV | Platform Citizenship | ✅ PASS | `pokedex/docker-compose.yml` is source of truth; services `pokedex-db`/`pokedex-api`/`pokedex-web`; host ports 3050/5050/5437 (constitution defaults, verified free); app-specific env in pokedex's own env files; stack-facing `pokedex.env` at platform root |
